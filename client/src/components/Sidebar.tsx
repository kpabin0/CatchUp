...
import { _loggedInAdminItems, _loggedInUserItems, _navItems } from '../data/_navItems'
import { useEffect, useState } from 'react'
import { TbDashboard, TbMenu, TbHome, TbCricket, TbTrophy, TbNews, TbBallTennis, TbLogout, TbPlus, TbEdit, TbLogin, TbBinoculars, TbBrandTeams, TbUser, TbHome2, TbQuestionMark, TbListDetails } from "react-icons/tb";
import { ISideNavItem } from '../utils/ITypes';
import { checkAdminStatus, loggedInStatus } from '../utils/utils';
import Logo from './Logo';
...
export function getIcon(name: string, style: string = iconStyle) {
    switch (name) {
        ...
        case "About Us":
            return <TbQuestionMark className={style} />
        case "Player Status":
            return <TbListDetails className={style} />
        case "Create Team":
        case "Create Tournament":
            return <TbPlus className={style} />
        case "Edit Team":
